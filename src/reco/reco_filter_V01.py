...
class RecoFilter(object):
    ...
    def sgnalToNoise(self, imagect):
        image = imagect.image()
        s = int(round(image.shape[0]*0.7))
        image_slice = image[s,:,:]
        image_slicef = np.float32(image_slice)
        
        
        #sobelx = cv2.Sobel(image_slicef,cv2.CV_32F,1,0,ksize=3)
        
        kernelx = np.array((-1,1),np.float32)
        filtx = cv2.filter2D(image_slicef, cv2.CV_32F, kernelx)
        IDXx = np.where((filtx>0) & (filtx<130))
        
        # kernely = np.array((-1,1),np.float32).transpose()
        # filty = cv2.filter2D(image_slicef, cv2.CV_32F, kernel)
        # #IDX = np.where(filt>0)
        
        noise = np.abs(filtx[IDXx]).mean()
        
        #noise = np.abs(sobelx).mean()
        
        return noise
    
    def extractSignalToNoise(self, settings, NumSamples=None):

        df_master = pd.read_excel(settings['filepath_master'], sheet_name = 'MASTER' + '_' + settings['date'], index_col=0)
        
        # Read fourier coefficients
        columns =['SeriesInstanceUID', 'Count', 'CLASS', 'RECO_CLASS', 'Features']
        dfSN = pd.DataFrame(columns=columns)
        ...
        for index, row in df_master[start:end].iterrows():
            print('index', index)   
            ...
            if RFCLabel=='CACS':
                ...
                imagect = series.image

                if len(imagect.image().shape)==3:                 
                    shape = imagect.image().shape
                    feature = self.sgnalToNoise(imagect) 
                    print('feature', feature)
                    #print('RECO_CLASS', RECO_CLASS)
                    #print('Count', shape[0])
                    dfSN = dfSN.append({'SeriesInstanceUID': SeriesInstanceUID, 
                                                  'Count': shape[0],
                                                  'CLASS': RFCLabel,
            ...
            if index % 500 == 0:
                dfSN.to_pickle(settings['filepath_sn'])
        dfSN.to_pickle(settings['filepath_sn'])
    ...
    def trainNoiseTree(self, settings):   
        ...
        dfSN.reset_index(inplace=True, drop=True)
        
        X = np.zeros((dfSN.shape[0], 2))
        for index, row in dfSN.iterrows():
            #print(row['Features'].shape)
            X[index,:] = [row['Features'], row['Count']]
        recoClassRF = defaultdict(lambda: -1, {'UNDEFINED':0, 'FBP': 1, 'IR': 2})
        Y = [recoClassRF[x] for x in list(dfSN['RECO_CLASS'])]
        
        #clfRF, confidence, C, ACC, pred = self.classifierTree(X, Y)
    ...
    def classifierTree(self, X, Y):
        ...
        # df = pd.read_pickle(settings['filepath_sn'])
        # Y = (df['RECO_CLASS']=='FBP')*1
        # X = df[['Count', 'Features']].to_numpy()
    
        # # Train random forest
        # #clfRF = RandomForestClassifier(max_depth=10, n_estimators=NumTrees)
        # clfRF = DecisionTreeClassifier()
        # clfRF = clfRF.fit(X, Y)
        
        # df = pd.read_pickle(settings['filepath_sn'])
        # Y = ((df['RECO_CLASS']=='FBP')*1).to_numpy()
        # X = df[['Count', 'Features']].to_numpy()
        ...
        return clf, confidence, C, ACC, pred
